...
import { getUsers, addComment, updateTaskPriority, type Task, type Priority } from "@/lib/store";
...
export function TaskDetailDialog({ task, open, onOpenChange, onRefresh }: Props) {
  const { user } = useAuth();
  ...
  const handleAddComment = () => {
    ...
    addComment(task.id, user.id, commentText.trim());
    setCommentText("");
    onRefresh();
  };
